...
statement = '''update user_clone_group set name = "%s" where name = "mdmpts" and id = (select a.id from user_clone_group a, mobile_client_info b where b.udid = "%s" and b.ucgID = a.id)'''
statement2 = '''select name, comment from user_clone_group where name = "mdmpts" and type = 24''' 
...
for i in my_list:
	...
	print "" 
    	cursor.execute(statement % ("TO BE DELETED",i[0]))
