...
#Reads Filewave device names values from the MDM DB to a CSV file and from there writes them to the Clients view for iOS devices.
#
...
# This is just a help to see how to do it from postgres if it is needed to be created manually:

#   \Copy (select device_id,device_name from generic_genericclient where device_product_name like '%iPad%') To '/usr/local/etc/naming.csv' With CSV;
...
k = 0
statement = """
            select device_id, device_name from generic_genericclient where device_product_name like '%iPad%'
            """
...
pos = 0
for i in record:
        temp=list(chunker(i,2))        # [1,2,3,4] --> [(1,2),(3,4)] Creates tuples for the uuid - device_name pairs.
        for p in temp:
                f.write(p[0]+","+p[1])
                f.write("\n")
f.close()
...
statement = '''update user_clone_group set name = "%s" where id = (select a.id from user_clone_group a, mobile_client_info b where b.udid = "%s" and b.ucgID = a.id)'''
