...
with open('/tmp/ipads.csv', 'rb') as f:
    reader = csv.reader(f)
    my_list = list(reader)
...
statement = '''update admin.user_clone_group set comment = 'TO BE DELETED' where type =24 and id = (select id from admin.user_clone_group where serial_or_mac = '%s')'''


for i in udids:
    print "To Be Deleted Device with Serial : ","\t-->\t",str(i)
