...
defaults read ~/Library/Preferences/com.apple.systemuiserver | grep -i "menu extras" | sed 's/^[^"]*"//; s/".*//' > /fwtemp/wifitmp.txt
...
for menuExtra in "${lines[@]}"; do
	...
	then
		...
		then
			...
			/usr/libexec/PlistBuddy -c "Delete :menuExtras:$c" ~/Library/Preferences/com.apple.systemuiserver.plist
			killall cfprefsd
			sleep 2
