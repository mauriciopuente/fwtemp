...
#Reads Serial numbers values from the MDM DB to a CSV file and from there writes them to the Clients view for iOS devices.
#
...
pos = 0
for i in record:
        temp=list(chunker(i,2))        # [1,2,3,4] --> [(1,2),(3,4)] Creates tuples for the uuid - serial_number pairs.
        for p in temp:
                f.write(p[0]+","+p[1])
                f.write("\n")
f.close()
