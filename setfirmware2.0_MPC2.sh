...
# Changes by Mauricio Puente
# Version 1.5 : 03-13-2016 - Added support for El Capitan, fixed the issue where the FWclient would get stuck running this script.
# Version 1.6 : 09-09-2016 - Added Recovery Partition verification so it doesn't say that it correctly setup the Firmware password when there isn't a Recovery partition.
...
function openrecovery {
    ...
    if [ $? == 0 ]; then
        ...
        if [ ${getXver} -eq 8 ];
          ...
          echo "FileWave Firmware Password Status: OK, Recoverydisk Opened " >> /var/log/fwcld.log
            ...
            then
                ...
                /usr/bin/hdiutil attach -quiet -nobrowse "$recoverydmg"
          echo "FileWave Firmware Password Status: OK, Recoverydisk Opened " >> /var/log/fwcld.log
            elif [ ${getXver} -eq 10 ] || [ ${getXver} -eq 11 ] || [ ${getXver} -eq 12 ]
            then
                /usr/sbin/diskutil mount "$recoverydiskname"
                /usr/bin/hdiutil attach -quiet -nobrowse "$recoverydmg"
          echo "FileWave Firmware Password Status: OK, Recoverydisk Opened " >> /var/log/fwcld.log
        ...
        echo "There is NOT a recovery partition here !!!" >> /var/log/fwcld.log
        exit 1
    fi
}
